...
Clusters.prototype.getColRowList = function() {
  var cook = '';
  for (var r in this.list) {
    var rect = this.list[r];
    var pos1 = new LatLngToColRow(rect._marker1.getPosition());
    var pos2 = new LatLngToColRow(rect._marker2.getPosition());
    cook += pos1.col + ',' + pos1.row + ',' + pos2.col + ',' + pos2.row + '|';
  }
  return cook;
}
...
function ClusterControl(map, div) {
  ...
  delObject.value = "Delete mark";
  div.appendChild(delObject);
  google.maps.event.addDomListener(delObject, 'click', function() {
    setDeleteObject = true;
  });

  var saveMap = document.createElement("input");
  saveMap.type = "button";
  saveMap.value = "Save marks";
  div.appendChild(saveMap);
  google.maps.event.addDomListener(saveMap, 'click', function() {
	var id = prompt('Save current markers/clusters as', 'common');
	if (id && id != '') {
	  saveMapToFile(id);
	}
  });

  var restoreMap = document.createElement("input");
  restoreMap.type = "button";
  restoreMap.value = "Load marks";
  div.appendChild(restoreMap);
  google.maps.event.addDomListener(restoreMap, 'click', function() {
	var id = prompt('Load markers/clusters from', 'common');
	if (id && id != '') {
	  restoreMapFromFile(id);
	}
  });
  ...
  var infoAll = document.createElement("input");
  infoAll.type = "button";
  infoAll.value = "All clusters";
  div.appendChild(infoAll);
  google.maps.event.addDomListener(infoAll, 'click', function() {
	//$(infoAll).toggle(clusterInfoAll, function(){$('#info').hide()});
	clusterInfoAll();
  });

  var searchDiv = document.createElement("div");
  searchDiv.style.textAlign = 'right';
  searchDiv.style.width = "650px";
...
function saveMapToFile(id) {
  var marks = markers.save();
  var clusts = clusters.save();
  var clustsCR = clusters.getColRowList();
  //alert(clustsCR);
  $.getJSON(URL_PREFIX + "/hyper/json?module=googlemap&method=saveMap&name="+config.name
			+"&markers="+marks+"&clusters="+clusts+"&idxclusters="+clustsCR+"&id="+id,
			function (data) {
	//alert(data['debug']);
	if (data['exception']) alert(data['exception']);
	else alert('Saved markers/clusters as ' + id);
  });
}

function restoreMapFromFile(id) {
//  var w = window.open('', '_blank', 'width=500,height=500,resizable=yes,toolbar=no,location=no,menubar=no');
  $.getJSON(URL_PREFIX + "/hyper/json?module=googlemap&method=restoreMap&name="+config.name+"&id="+id, function (data) {
	var marks = markers.save();
	var clusts = clusters.save();
	if (data['exception']) alert(data['exception']);
	else {
	  if (data['markers']) {
		marks = marks == null ? data['markers'] : marks += data['markers'];
		markers.cookie.set(marks);
		markers.removeAll();
		markers.restore();
	  }
	  if (data['clusters']) {
		clusts = clusts == null ? data['clusters'] : clusts += data['clusters'];
		clusters.cookie.set(clusts);
		clusters.removeAll();
		clusters.restore();
	  }
	  if (!data['markers'] && !data['clusters']) {
		//w.close();
		alert('Nothing to load');
	  } else {
		//w.location.replace(URL_PREFIX + "/hyper?mako=gmapallinfo&map="+config.name+"&mapid="+config.mapId+"&load="+id);
	  }
	}
	//alert(data['debug']);
  });
...
var winfo = null;
function clusterInfoAll() {
  var done = clusters.list.length;
  if (done == 0) return;
  ...
  if (!winfo || winfo.closed) {
    winfo = window.open('', 'winfo', 'width=500,height=500,resizable=yes,scrollbars=yes,toolbar=no,location=no,menubar=no');
  }
  winfo.focus();
  winfo.document.write('<html><head><style>.clusterinfo{height:auto}</style></head><body>');
//  winfo.document.close();
//  html = $(winfo.document.body);
//  html.empty();
  for (var i in clusters.list) {
	var cluster = clusters.list[i];
	if (!cluster) continue;
	...
	if (!cluster._hasInfo) {
	  ...
	  var closure = function(mycluster) {	  
	    return function (info) {
		  //alert(this.index);
		  mycluster._infoWindow.setContent(info);
		  mycluster._hasInfo = true;
  
		  //html.append(info);
		  //html.append('<hr>');
		  //html.children('.clusterinfo:last').height('auto');
		  winfo.document.write(info);
		  if (--done == 0) {
		    winfo.document.write('</body></html>');
		    winfo.document.close();
		    $(winfo.document.body).children('.clusterinfo').height('auto');
		  } else {
		    winfo.document.write('<hr>');
		  }
	    }
	  }
	  
	  $.get(URL_PREFIX + "/hyper?mako=gmapclusterinfo&from="+from+"&to="+to+"&map="+config.name+"&mapid="+config.mapId, closure(cluster));
	  
	} else {
		//html.append(cluster._infoWindow.getContent());
		//html.append('<hr>');
		//html.children('.clusterinfo:last').height('auto');
		winfo.document.write(cluster._infoWindow.getContent());
		if (--done == 0) {
		  winfo.document.write('</body></html>');
		  winfo.document.close();
		  $(winfo.document.body).children('.clusterinfo').height('auto');
