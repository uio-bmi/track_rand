...
class Motif:
    ...
    def dualBestScoreSeqEnd(self,seq, mSeq):
        """
        m.bestscanseq(seq) -- Return score,sequence of the best match to the motif in the supplied sequence
        """
        matches,endpoints,scores = self.scan(seq,-100)
        mMatches,mEndpoints,mScores = self.scan(mSeq,-100)
        
        bestIndx = sorted(range(len(scores)), key = seq.__getitem__)[-1]
        mBestIndx = sorted(range(len(mScores)), key = seq.__getitem__)[-1]
        
        return [(scores[bestIndx], mScores[bestIndx], matches[bestIndx], mMatches[bestIndx], endpoints[bestIndx]), (mScores[mBestIndx], scores[mBestIndx], mMatches[mBestIndx], matches[mBestIndx], mEndpoints[mBestIndx])]
...
def print_motifs(motifs,kmer_count=20,istart=0):
    ...
    for m in motifs:
        ...
        m._print_ll()
        #print "Probability matrix for Motif %3d %s"%(i,m)
        #m._print_p()
        print "Sequence Logo"
        m._print_bits()
        ...
        if m.binomial!=None:  print ' Bi: %5.2f'%nlog10(m.binomial),
