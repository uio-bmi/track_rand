...
#Input:
#	data: data frame with at least three columns. Rows represent probes, the first column gives arm numbers, the second gives local probe positions, while the rest of the column(s) give estimated copy numbers for one or more samples
# 	segments: 

plot.genome <- function(data=NULL,segments=NULL,winsoutliers=NULL,sampleID=NULL,xaxis="pos",unit=NULL,layout=c(1,1),dir.print=NULL,segments2=NULL,...){
	
	if(!is.null(data)||!is.null(segments)){
		#First extract relevant information from input data:
		if(!is.null(data)){
			#Check data and extract:
			ext.data <- extractFromData(data,chrom=NULL,sampleID,winsoutliers,type="genome")
			arm <- ext.data$arm
		...
		if(!is.null(segments)){
			#Check segments, and extract:
			ext.seg <- extractFromSegments(segments,chrom=NULL,sampleID=sampleID,type="genome")
			...
			if(!is.null(segments2)){
				ext.seg2 <- extractFromSegments(segments=segments2,chrom=NULL,sampleID=sampleID,type="genome")
				segments2 <- ext.seg2$segments
			}
		}#endif
		
		#Find chromosomenumber corresponding to arm number:
		chrom <- getChrom(arm)
		...
		arg <- getPlotParameters(type="sample",nc=nc,sampleID=sampleID,chrom=chrom,...)
		...
		for(i in 1:nSample){
			...
			par(fig=fig.c,new=new,oma=c(0,0,1,0),mar=c(0.2,0.2,0.3,0.2))
			frame.c <- list(left=frames$left[clm],right=frames$right[clm],bot=frames$bot[row],top=frames$top[row])

			#Plotting:
			if(is.null(segments)){
				#Plot only observed data:
				#Find data for sample i:
				index <- which(colnames(y)==sampleID[i])
	
				plotObs(y,k=NULL,index=index,winsoutliers,xaxis=xaxis,position=position,type="genome",chromosomes=chrom,frame=frame.c,new=new,pos.unit=unit,op=arg)		
			}else{
				if(is.null(data)){
					#Plot only segments:
					index <- which(segments[,1]==sampleID[i])
					
					#Get max and min for segments2:
					seg2.lim <- NULL
					if(!is.null(segments2)){
						index2 <- which(segments2[,1]==sampleID[i])
						seg2.lim <- get.seglim(segments2,arg$equalRange,index=index2)
					}#endif

					#Plot the segments:
					plotSegments(segments,type="genome",index=index,chromosomes=chrom,xaxis=xaxis,add=FALSE,col=arg$seg.col[1],lty=arg$seg.lty[1],lwd=arg$seg.lwd[1],
						frame=frame.c,new=new,pos.unit=unit,seg2.lim=seg2.lim,op=arg)

					if(!is.null(segments2)){
						#Plot second segmentation on top (e.g. results from Multipcf)
						plotSegments(segments2,type="genome",index=index2,xaxis=xaxis,add=TRUE,col=arg$seg.col[2],lty=arg$seg.lty[2],lwd=arg$seg.lwd[2],pos.unit=unit,op=arg)

					}#endif

				}else{
					#Plot both:
					indexObs <- which(colnames(y)==sampleID[i])
					indexSeg <- which(segments[,1]==sampleID[i])
					
	
					#Check min and max for segments and segments2 to make sure all segments are included in plot:
					seg.lim <- get.seglim(segments,arg$equalRange,index=indexSeg)
					if(!is.null(segments2)){
						indexSeg2 <- which(segments2[,1]==sampleID[i])
						
						seg2.lim <- get.seglim(segments2,arg$equalRange,index=indexSeg2)
						seg.lim[1] <- min(seg.lim[1],seg2.lim[1])
						seg.lim[2] <- max(seg.lim[2],seg2.lim[2])
					}
					
					#Plot the observed data:
					plotObs(y,k=NULL,index=indexObs,winsoutliers,xaxis=xaxis,position=position,type="genome",chromosomes=chrom,frame=frame.c,new=new,pos.unit=unit,seg.lim=seg.lim,op=arg)
					
					#Plot the segments:
					plotSegments(segments,type="genome",index=indexSeg,xaxis=xaxis,add=TRUE,col=arg$seg.col[1],lty=arg$seg.lty[1],lwd=arg$seg.lwd[1],pos.unit=unit,op=arg)

					if(!is.null(segments2)){
						#Plot segments2 on top (e.g. results from Multipcf)
						plotSegments(segments2,type="genome",index=indexSeg2,xaxis=xaxis,add=TRUE,col=arg$seg.col[2],lty=arg$seg.lty[2],lwd=arg$seg.lwd[2],pos.unit=unit,op=arg)
...
#		q=NA,h=0,h.lty=5,h.lwd=2*f,h.col="darkgrey",ideo.frac=NA,cyto.text=FALSE,cex.cytotext=0.6,sepChrom=FALSE,
#		cex.chrom=0.7*f,plot.unit="mbp",seg.col=c("red","green"),seg.lty=c(1,1),seg.lwd=c(4*f,3*f),equalRange=TRUE)

	arg <- list(title="",col="dodgerblue",wins.col="magenta",pch=".",wins.pch="*", ylab=expression("FDR-corrected P-values"),
		xlab="Genome position (Mbps)",main=NULL,xlim=NULL,ylim=NULL,f=ff[nc],cex=2*f,wins.cex=0.4,cex.lab=1*f,cex.main=1.2*f,
		cex.axis=0.9*f,las=1,at.x=NULL,at.y=NULL, main.line=0.6*f,mgp=NULL,tcl=-0.3*f,equalRange=TRUE,
		q=NA,h=0.1,h.lty=5,h.lwd=2*f,h.col="black",ideo.frac=NA,cyto.text=FALSE,cex.cytotext=0.6,sepChrom=TRUE,
		cex.chrom=0.7*f,plot.unit="mbp",seg.col=c("red","green"),seg.lty=c(1,1),seg.lwd=c(2*f,2*f),equalRange=TRUE)
