...
#        if fn.endswith('.gtrack.gz'):
#            return GzipGtrackGenomeElementSource.__new__(GzipGtrackGenomeElementSource)
#        return GtrackGenomeElementSource.__new__(GtrackGenomeElementSource)
#
#
#class HbGtrackGenomeElementSource(GenomeElementSource):
#    def __new__(cls, fn, *args, **kwArgs):
#        if fn.endswith('.gtrack.gz'):
#            return HbGzipGtrackGenomeElementSource.__new__(HbGzipGtrackGenomeElementSource)
#        return HbGtrackGenomeElementSource.__new__(HbGtrackGenomeElementSource)
...
class GtrackGenomeElementSource(GenomeElementSource):
    ...
    GTRACK_SUBTYPE_REPO = 'http://gtrack.no/'
    ...
    def _parseColumnSpecLine(self, line):
        self._createColumnSpec([col.strip() for col in line[3:].split('\t')])
    ...
    def _updateHeadersAccordingToSubtype(self, overrideDefaults=True):
        ...
        if self._headerDict['subtype url'] == '':
            from gold.util.CommonFunctions import getFileSuffix
            suffix = self._suffix
            if not suffix:
                suffix = getFileSuffix(self._fn)

            suffixBasedSubTypeUrl = self._getSuffixBasedGTrackSubtypeUrl(suffix)
            if suffixBasedSubTypeUrl:
                self._headerDict['subtype url'] = suffixBasedSubTypeUrl
            else:
                return
        ...
        if self._defaultColumnSpec:
            self._createColumnSpec(subtypeGESource.getColumns(orig=True))
        else:
            self.checkColumnSpecCorrespondence(subtypeGESource.getColumns(orig=False), \
                                               self.getColumns(orig=True), subtypeAdherence)

    @classmethod
    def suffixBasedGTrackSubtypeExists(cls, suffix):
        return cls._getSuffixBasedGTrackSubtypeUrl(suffix) is not None

    @classmethod
    def _getSuffixBasedGTrackSubtypeUrl(cls, suffix):
        if suffix not in ['gtrack', 'gz']:
            testSubtypeUrl = cls.GTRACK_SUBTYPE_REPO + suffix.lower() + '.gtrack'
            #print testSubtypeUrl

            try:
                cls._getSubtypeFileObject(testSubtypeUrl)
                return testSubtypeUrl
            except:
                pass
    ...
    def _getSubtypeGESource(cls, url, printWarnings):
        ...
        return GtrackGenomeElementSource('subtype.gtrack', subtype=True, \
                                         printWarnings=printWarnings, \
                                         strToUseInsteadOfFn=subtypeContents)

    @classmethod
    def _getSubtypeFileObject(cls, url):
        try:
            if url.startswith(cls.GTRACK_SUBTYPE_REPO):
                from config.Config import STATIC_PATH
                fn = os.sep.join([STATIC_PATH, 'gtrack', url[len(cls.GTRACK_SUBTYPE_REPO):]])
    ...
    def _downloadSubtypeFileContents(cls, url):
        fileObj = cls._getSubtypeFileObject(url)
        return fileObj.read()
    ...
    def _parseDataLine(self, line):
        ...
        for curLine in dataLines:
            ...
            cols = [x.strip() for x in curLine.split('\t')]
            for col in cols:
                self._checkCharUsageOfPhrase(col)
...
class HbGtrackGenomeElementSource(GtrackGenomeElementSource):
    _RETURN_NUMPY_TYPES = True

    _addsStartElementToDenseIntervals = True

    def __init__(self, fn, *args, **kwArgs):
        GtrackGenomeElementSource.__init__(self, fn, *args, **kwArgs)
    ...
    def _checkHeaderLines(self):
        GtrackGenomeElementSource._checkHeaderLines(self)
    ...
    def _renameExtraKeyIfNeeded(self, key):
        key = GtrackGenomeElementSource._renameExtraKeyIfNeeded(self, key)
    ...
    def _parseBoundingRegionLine(self, line):
        GtrackGenomeElementSource._parseBoundingRegionLine(self, line)
    ...
    def _handleBoundingRegionLineTypeB(self, br):
        GtrackGenomeElementSource._handleBoundingRegionLineTypeB(self, br)
...
class GzipGtrackGenomeElementSource(GtrackGenomeElementSource):
