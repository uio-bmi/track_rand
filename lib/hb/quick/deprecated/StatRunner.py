    ...
    def _checkNumUserBinsIsValid(self):
        ...
        if numUserBins < 1:
            ...
            raise InvalidFormatError(
                'Maximum number of user bins exceeded - Maximum: ' + str(MAX_NUM_USER_BINS) + ', Requested: ' + str(
                    numUserBins))
            # return False
        return True
...
class Progress():
    ...
    def addCount(self, newCounts=1):
        ...
        for percentage in range(100 * self._currCount / self._totalCount + 1,
                                100 * (self._currCount + newCounts) / self._totalCount + 1):
            print '.',
            if percentage % 10 == 0:
                print percentage, '%'
    ...
    def globalAnalysisEnded(self):
        ...
        if self._startGlobalTime is None:
            logMessage('Called globalAnalysisEnded without globalAnalysisStarted being called before.',
                       level=logging.WARN)
...
class SequentialMcProgress(Progress):
    ...
    def __init__(self, totalCount, printProgress, description=None, numSamplesPerIteration=None, **ignoredKwArgs):
        ...
        self._numSamplesPerIteration = int(
            numSamplesPerIteration) if numSamplesPerIteration is not None else 100  # RandomizationManagerStatUnsplittable.NUM_SAMPLES_PER_CHUNK
        self._description += '\nComputing p-values by %s, printing one dot per %i samples across bins' % (
        self.SCHEME_NAME, self._numSamplesPerIteration)
    ...
    def addFullLocalAnalysisIteration(self, numBinsNotDetermined):
        ...
        if self._numIterations % 10 == 0:
            self.printMessage('%.1f%% of bins determined (%i samples per bin)' \
                              % (100.0 * (self._totalCount - numBinsNotDetermined) / self._totalCount,
                                 self._numIterations * self._numSamplesPerIteration))
    ...
    def addGlobalAnalysisIteration(self, mValue, mThreshold, pValue, pThreshold):
        ...
        if self._numIterations % 10 == 0:
            self.printMessage(
                'Reached %i samples for global analysis (Extreme samples:%i towards %i, P-value:%s towards %s)' % (
                self._numIterations * self._numSamplesPerIteration, mValue, mThreshold, pValue, pThreshold))
