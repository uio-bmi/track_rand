...
class UnionOfGSuiteTracksTool(GeneralGuiTool, GenomeMixin, DebugMixin):
    ...
    def getToolName():
        ...
        page.
        '''
        return "Generate a union track from all the tracks in a GSuite"
    ...
    def execute(cls, choices, galaxyFn=None, username=''):
        ...
        sf = GalaxyRunSpecificFile(['union'], galaxyFn)
        #
        # from gold.application.HBAPI import doAnalysis
        # from quick.application.UserBinSource import GlobalBinSource
        # from quick.statistic.GenericGenomeElementWriterStat import GenericGenomeElementWriterStat
        ...
        analysisSpec.addParameter("quotedOutTrackFn", quote(extraOutFn, safe=""))
        analysisSpec.addParameter("generatorStatistic", UnionTrackOfGSuiteTracksStat.__name__)
        # bins = GlobalBinSource(choices.genome)
        ...
        # analysisSpec = AnalysisSpec(UnionTrackOfGSuiteTracksStat)
        trackNameList = [Track(track.trackName) for track in gSuite.allTracks()]
        bins = GlobalBinSource(choices.genome)
        doAnalysis(analysisSpec, bins, trackNameList)
        print 'Done. The generated track is available in the "Union track" history element.'
        # print sf.getLink("TRACK HERE")
        # print "Written elements: ", res.getGlobalResult()
