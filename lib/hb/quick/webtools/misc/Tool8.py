...
class Tool8(GeneralGuiTool):
    ...
    def getInputBoxNames():
        ...
        '''
        return [('Select tabular file from history', 'history'),
                ('Number of lines to preview', 'numLines'),
                ('First three lines of file', 'table')]
    ...
    @staticmethod
    def getOptionsBoxNumLines(prevChoices): # Alternatively: getOptionsBox2()
        '''
    ...
    def getOptionsBoxTable(cls, prevChoices):
        try:
            numLines = int(prevChoices.numLines)
        except:
            return

        if prevChoices.history:
            contents = cls._getContentsOfHistoryFile(prevChoices.history)
            if contents:
                maxLen = max(len(row) for row in contents)
                header = [['Col %s' % (i+1) for i in range(maxLen)]]
                return header + contents[:numLines]
    #@staticmethod
    ...
    def validateAndReturnErrors(choices):
        ...
        if not choices.history:
            return 'Please choose a tabular file from history'

        try:
            int(choices.numLines)
        except:
            return "'%s' is not an integer" % choices.numLines
