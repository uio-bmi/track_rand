...
class Tool9(GeneralGuiTool):
    @staticmethod
    def getToolName():
        return "Concatenate results of multiple history items (v3)"
    ...
    def getInputBoxNames():
        ...
        return ['Select Histories','List available result statistics','Select result statistic', 'Use local or global results','Num columns for line wrapping (leave blank for no wrapping)','Impute missing values', 'Specify header for output table', 'Column labels for output file, pipe-separated (|) if multiple output columns', 'Allow missing values'] #,'Ensure validity'
    ...
    def getOptionsBox8(prevChoices):
        return '' if prevChoices[6]=='Yes' else None
    ...
    def execute(cls, choices, galaxyFn=None, username=''):
        ...
        #Note: Imputation of NAs is partially supported, but should be checked before being activately used
        if choices[8]=='Yes':
            imputeNAs = True
        elif choices[8]=='No':
            imputeNAs = False
