...
from quick.extra.exam.ExamResultsAnalysis import ExamScoresOverview,\
...
class ExamScoresOverviewTool(GeneralGuiTool):
    @staticmethod
    def getToolName():
        '''
...
#         return ''
    ...
    def execute(choices, galaxyFn=None, username=''):
        ...
        examAnalysis = ExamScoresOverview(resultsFN, galaxyFn)
        examAnalysis.run()
    ...
    def validateAndReturnErrors(choices):
        ...
        errors = ExamResultsValidator.validateExamResultsDataFile(resultsFN, 
                                                                  ExamScoresOverview.REQUIRED_COLUMNS, 
                                                                  ExamScoresOverview.OPTIONAL_COLUMNS)
        if errors:
            if len(errors) > 5:
                errors = errors[0:5]
