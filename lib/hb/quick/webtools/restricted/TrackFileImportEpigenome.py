...
class TrackFileImportEpigenome(GeneralGuiTool):
    ...
    colList = DB.getTableCols('file_epigenome')   
    for col in colList:
    ...
    def getInputBoxNames():
        ...
        attrBoxes = []
        for i in xrange(len(TrackFileImportEpigenome.ATTRIBUTES)):
            attrBoxes.append(('<b>Select Attribute:</b>',\
                 'attributeList%s' % i))
            attrBoxes.append((35*'&#160'+'Select Value: ',\
                 'valueList%s' % i))
            attrBoxes.append(('Range ' + TrackFileImportEpigenome.ATTRIBUTES[i],\
                 'variableRange%s' % i))
    ...
    #    colsList = []
    #    for col in TrackFileImportEpigenome.ATTRIBUTES:
    ...
    def _getVariableRangeBox(cls, prevChoices, index):
        ...
        if not TrackFileImportEpigenome.ATTRIBUTES[index] in selected_attrs:
           return
    ...
    def _getVariableListBox(cls, prevChoices, index):
        ...
        #if val == None:
        #   return
        colsList = TrackFileImportEpigenome.ATTRIBUTES[:]
        colsList.insert(0,'--None--')
        if index == 0:
           return colsList
        for i in xrange(len(TrackFileImportEpigenome.ATTRIBUTES)):
            ...
            #   colsList.append(TrackFileImportEpigenome.ATTRIBUTES[i])
    ...
    def _getVariableBox(cls, prevChoices, index):
        ...
        #col = TrackFileImportEpigenome.ATTRIBUTES[index]
        ...
        for i in xrange(len(TrackFileImportEpigenome.ATTRIBUTES)):
            ...
            if col in [None,'--None--'] or val in ['--Everything--','--Range--',None,'']:
               continue
            col = TrackFileImportEpigenome.DB.correctColumNames([col])[0]
        ...
        rows = TrackFileImportEpigenome.DB.getColGroup('file_epigenome',\
                                               value,\
    ...
    def getOptionsBoxResults(prevChoices): 
        if prevChoices.download in [None,'--NO--']:
           return
        cols,colListString = TrackFileImportEpigenome.getColListString()
        rows = TrackFileImportEpigenome.getRows(prevChoices,colListString)
        List = []
        for row in rows:
            ...
            #    line+= str(row[i])+'\t|'
            #line = line.strip('|')
            TrackFileImportEpigenome.RESULT_FILES_URLS.append(row[0].replace('http://','ftp://'))
    ...
    def execute(cls,choices, galaxyFn=None, username=''):
        ...
        rows = TrackFileImportEpigenome.getRows(choices,colListString)
        
        outFile = open(galaxyFn, 'w')
        
        
        if len(TrackFileImportEpigenome.RESULT_FILES_URLS) > 0:
         
          
          i = 0
          for x,selected in choices.results.iteritems():
             if selected:
                TrackFileImportEpigenome.SELECTED_FILES_INDEXES.append(i)  
             i +=1
          import ftplib
          for i in TrackFileImportEpigenome.SELECTED_FILES_INDEXES:
              
              url = TrackFileImportEpigenome.RESULT_FILES_URLS[i]
        ...
        if choices.outFormat == 'Html':
           TrackFileImportEpigenome.printHtml(cols,rows,colListString,outFile)
        else:
           TrackFileImportEpigenome.printText(cols,rows,colListString,outFile)
        
    @staticmethod
    def getColListString():
        cols = TrackFileImportEpigenome.DB.correctColumNames\
        (TrackFileImportEpigenome.DB.getTableCols('file_epigenome'))
        colListString = ''
    ...
    def getRows(choices,colListString):
        ...
        query = "select "+colListString+" from file_epigenome "
        if WHERE != '':
           query += "where "+WHERE+";"
        else:
           query += ";"
        
        rows = TrackFileImportEpigenome.DB.runQuery(query)
    ...
    def printHtml(cols,rows,colListString,outFile):
        ...
        if len(TrackFileImportEpigenome.SELECTED_FILES_INDEXES)>0:
           arr = TrackFileImportEpigenome.SELECTED_FILES_INDEXES
        else:
           arr = range(len(rows))
        for i in arr:
            row = rows[i]
...
#TrackFileImportEpigenome.DB.createTableFromList('temp',['attr'])
TrackFileImportEpigenome.setupVariableBoxFunctions()
