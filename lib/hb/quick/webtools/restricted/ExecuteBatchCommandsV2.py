...
class ExecuteBatchCommandsV2(GeneralGuiTool):
    @staticmethod
    def getToolName():
        '''
    ...
    def getInputBoxNames():
        ...
        '''
        return ['Select choice', 'Insert batch commands (one line per run)', 'select dataset', ''] #Alternatively: [ ('box1','1'), ('box2','2') ]
    ...
    INPUT_FIELD = 'write commands in input field'
    HIST_DATA = 'use commands from dataset'

    @classmethod
    def getOptionsBox1(cls):
        return ['-----  Select  -----', cls.INPUT_FIELD, cls.HIST_DATA]
    ...
    def getOptionsBox2(cls, prevChoices):
        ...
        '''
        if prevChoices[0] == cls.INPUT_FIELD:
            return '', 14, False

    @classmethod
    def getOptionsBox3(cls, prevChoices):
        if prevChoices[0] == cls.HIST_DATA:
            return '__history__'

    @classmethod
    def getOptionsBox4(cls, prevChoices):
        if prevChoices[1] and len(prevChoices[1].split('\n'))>1:
            resdict = dict()
            batchTemplate = '''old_values=%%7B%%7D&datatype=customhtml&mako=generictool&tool_id=hb_execute_batch_commands_v2&tool_name=hb_execute_batch_commands_v2&URL=http://dummy&box1=write commands in input field&box2=%s'''
            for i, v in enumerate(prevChoices[1].split('\n')):
                if v:
                    resdict[str(i)] = batchTemplate %  v
        ...
        alert("Added "+ String(count) +" batch runs to your history..");
        } );</script>'''
            return '__rawstr__',jCode % repr(resdict), False
...
mm8|chr17:3m-|5m|@trackNames1|@trackName2|@stat1/@stat2'''), None]
    ...
    def execute(cls, choices, galaxyFn=None, username=''):
        ...
        if choices[0] == cls.INPUT_FIELD:
            ...
            batchStr = open(ExternalTrackManager.extractFnFromGalaxyTN(choices[2].split(':'))).read()
    ...
    def validateAndReturnErrors(cls, choices):
        ...
        '''
        if choices[0] not in [cls.INPUT_FIELD, cls.HIST_DATA]:
            return ''

        elif choices[0]== cls.INPUT_FIELD and choices[1].strip() == '':
            return ''

        elif choices[0] == cls.HIST_DATA and choices[1].strip() == '':
            return ''
        return None
