    ...
    for data_dict in job_params[ 'output_data' ]:
        ...
        try:
            if not URL_method or URL_method == 'get':
                page = urllib.urlopen( urllib.unquote(cur_URL) )
        ...
        with open( cur_filename, 'w' ) as f:
            if params.get('cm_data_type') == 'bed':
                f.write('##gtrack version: 1.0\n##track type: valued segments\n##uninterrupted data lines: true\n###seqid\tstart\tend\tname\tvalue\tstrand\n')
            if params.get('cm_data_type') == 'wig':
                f.write('##gtrack version: 1.0\n##track type: valued segments\n##uninterrupted data lines: true\n###seqid\tstart\tend\tvalue\n')

        #do sniff stream for multi_byte
        try:
            cur_filename, is_multi_byte = sniff.stream_to_open_named_file( page, os.open( cur_filename, os.O_WRONLY | os.O_APPEND ), cur_filename )
        except Exception, e:
